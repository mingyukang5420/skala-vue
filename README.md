...
> 과제4~7은 이후 같은 화면 파일을 공유하도록 발전했고, 라이브 앱에서는 최종본(과제4~7 통합, "최종 제출" 탭)만 확인 가능합니다(`/exercise/4`, `/exercise/5`는 자동으로 최종본으로 리다이렉트). 사이트에 접속하면 기본으로 이 최종본 화면이 뜹니다. 각 단계에서 실제로 추가된 기능은 아래 스크린샷과 커밋 이력을 참고하세요.
...
- (추가) 과제4~6이 전부 같은 화면 파일을 공유하는 구조라, UI 라이브러리 적용도 자연스럽게 과제4~7 전체에 한 번에 반영된다. 타이틀도 "최종 제출 — Weather App (과제4~7 통합)"으로 갱신했다.
- (추가) 이 화면이 단계별 과제 중 하나가 아니라 최종 완성본이라는 걸 명확히 하려고, 상단 탭 라벨을 "과제4-7"에서 "최종 제출"로 바꾸고, 사이트 루트(`/`) 접속 시 기본으로 이 화면이 뜨도록 라우팅도 바꿨다.
